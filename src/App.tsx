...
function Guard({children}:{children:ReactNode}){
  const a=useAuth();
  if(a.loading)return <div className="loading full">Loading Hayat…</div>;
  if(!a.platformAdmin&&!a.provider)return <Navigate to="/login" replace/>;
  return <>{children}</>;
}
function DashboardEntry(){
  const a=useAuth();
  if(a.platformAdmin)return <Navigate to="/platform" replace/>;
  if(!a.provider)return <Navigate to="/login" replace/>;
  return <RoleDashboard/>;
}
function RoleOnly({roles,children}:{roles:string[];children:ReactNode}){
  const a=useAuth();
  if(a.platformAdmin||!a.provider||!roles.includes(a.provider.role))return <Navigate to="/" replace/>;
  return <>{children}</>;
}
function PermissionOnly({code,children}:{code:string;children:ReactNode}){
  const a=useAuth();
  if(a.platformAdmin||!hasPermission(a.provider,code))return <Navigate to="/" replace/>;
  return <>{children}</>;
}
function PlatformOnly({children}:{children:ReactNode}){
  const a=useAuth();
  if(!a.platformAdmin)return <Navigate to="/" replace/>;
  return <>{children}</>;
}
